...
function handleSearch(room){
    return function(nickname='', onlineState){
        ...
        return socketEmit('getRoomUsers')({nickname, onlineState, room}).then(ret => this.setState({users: ret, isLoading: false}))
    }
}
...
function RoomUsers(props){
    ...
    return (
        <div className = 'Profile-container'>
            <Header title = {language.participants} leftElement = {<i className = 'icon' onClick = {() => setRightManager({isShow: false})}>&#xe604;</i>}/>
            <ContactList />
        </div>
    )
}
