...
function RoomProfile(props){
    const {isCreater, _id, avatar, bulletin, createAt, name, creater, isLoading, handleChangeRoomInfo, handleExitRoom} = props;
    return (
        <div className = 'Profile-container'>
            <Header title = {language.groupInfo} leftElement = {<i className = 'icon' onClick = {() => setRightManager({isShow: false})}>&#xe604;</i>}/>
            {
                isLoading ? 
                <Loading/> : 
                    ...
                    <div className = 'Profile-section'>
                        ...
                        <div className = 'RoomProfile-info'>
                            <ContentEditableInput 
                                ...
                                defaultContent = {name || ''}
                                count = {20}
                                controllerBtn = {isCreater}
                                handleSubmit = {handleChangeRoomInfo?handleChangeRoomInfo('name'):null}
                            />
                            <p>{'Created at ' + timedeal.getYDHString(createAt)}</p>
                        </div>
                    </div>
